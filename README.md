# Hi, I'm Rishi Garg 👋

🎓 B.Tech in Chemical Engineering, IIT Guwahati  
💡 Interested in Machine Learning and Data Science  

---

## 👨‍💻 About Me

- Undergraduate student at IIT Guwahati (2024–Present)  
- Experienced in Machine Learning, Data Analysis, and Full-Stack Development  
- Passionate about building scalable and data-driven applications  
- Codeforces Max Rating: **1253 (Pupil)**  

---

## 🛠️ Technical Skills

### Programming Languages
- C/C++, Python, MATLAB  

### Libraries & Tools
- NumPy, Pandas, Matplotlib, Seaborn, Plotly  

### Machine Learning
- Scikit-learn, XGBoost  

### Web Development
- **Frontend:** HTML, CSS, JavaScript, React  
- **Backend:** Node.js, Express.js  

### Databases
- MongoDB, MySQL  

### Other Tools
- Git, Linux  

---

## 📌 Projects

### 🔹 Customer Churn Prediction System
- Built machine learning models on telecom dataset (7000+ records)  
- Performed exploratory data analysis and feature engineering  
- Achieved **84.7% accuracy** using ensemble methods  

### 🔹 Text-Based Risk Classification (SMS Spam Detection)
- Developed NLP-based classifier using TF-IDF  
- Applied feature engineering techniques  
- Achieved **F1-score of 0.92**  

### 🔹 Airline Management System
- Developed a full-stack flight booking platform  
- Implemented secure authentication and payment integration  
- Designed responsive UI and backend APIs  

### 🔹 Real-Time Communication Platform
- Built messaging backend using MERN stack  
- Implemented JWT-based authentication and REST APIs  

---

## 📚 Coursework
- Introduction to Computing  
- Computer-Aided Numerical Methods  
- Linear Algebra  
- Complex Analysis  
- Basic Econometrics  

---

## 📫 Contact

- Email: rishi.garg@iitg.ac.in  
- Alternate Email: rishigarg525@gmail.com  
- LinkedIn: https://linkedin.com/in/rishiigarg  
- Codeforces: https://codeforces.com/profile/RISHI214
- Leetcode: https://leetcode.com/u/zsVIMS8QlC/
- Codechef: https://www.codechef.com/users/imposter69

---

⭐ *Open to opportunities in Machine Learning, Data Science, and Software Development*
